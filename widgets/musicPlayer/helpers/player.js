...
export var Players = class Players {
  ...
  pick() {
    ...
    if (!this.list.includes(this.selected)) this.selected = null;
    ...
    if (AUTO_SWITCH_TO_MOST_RECENT) {
      if (this.activePlayers.length == 0) return this.selected;
  ...
  next() {
    ...
    if (AUTO_SWITCH_TO_MOST_RECENT) list = this.activePlayers;

    if (list.length < 2) return this.selected;

    let newIndex = list.indexOf(this.selected) + 1;

    if (this.selected == list[list.length - 1]) newIndex = 0;
